  ...
  for (const group of groups) {
    const { data: policy, error } = await supabase
      ...
      .insert({
        ...
        monthly_premium: group.monthly_premium,
        start_date: normalizeDate(group.start_date),
        ...
        status: group.status || group.contract_status || '정상',
      })
      .select('id')
      .single()
    ...
    const coverageRows = group.coverages.map((coverage: any) => ({
      ...
      coverage_name: coverage.coverage_name || coverage.coverage_name_original || coverage.name || '담보명 미확인',
